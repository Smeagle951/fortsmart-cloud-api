...
import { consumePairingSession, createPairingSession, getPairingSessionStatus, listTrustedDevices, revokePairingSession, revokeTrustedDevice, validateDeviceBinding, } from '../services/pairing.service.js';
import { getPool } from '../db/pool.js';
import { requireApiKey } from '../middleware/apiKeyAuth.js';
import { asyncHandler } from '../utils/asyncHandler.js';
import { jsonOk } from '../utils/response.js';
export const authRouter = Router();
const NOT_LINKED_MSG = 'API Key válida. Primeira sincronização ainda não vinculou uma fazenda.';
...
authRouter.post('/auth/api-key/regenerate', asyncHandler(async (req, res) => {
    const body = req.body;
    const apiKey = typeof body?.api_key === 'string' ? body.api_key.trim() : '';
    const farmId = typeof body?.farm_cloud_id === 'string' && body.farm_cloud_id.trim()
    ...
    if (!farmId)
        throw new HttpError('farm_id or farm_cloud_id is required', 400);
...
authRouter.post('/auth/api-key/validate', asyncHandler(async (req, res) => {
    const row = await authenticateBearer(req.headers.authorization);
    const body = req.body;
    const deviceId = String(req.headers['x-fortsmart-device-id'] ?? '').trim() ||
    ...
    const farmIdParam = typeof body?.farm_id === 'string'
        ? body.farm_id.trim()
        : body?.farm_id != null
    ...
    const pool = getPool();
    const { rows } = await pool.query(`SELECT id, local_id, name FROM farms
...
authRouter.post('/auth/pairing/create', requireApiKey, asyncHandler(async (req, res) => {
    const body = req.body;
    const farmCloudId = typeof body?.farm_cloud_id === 'string' ? body.farm_cloud_id.trim() : req.cloudAuth?.farmId ?? '';
    const desktopInstallationId = typeof body?.desktop_installation_id === 'string' ? body.desktop_installation_id.trim() : '';
...
authRouter.post('/auth/pairing/consume', asyncHandler(async (req, res) => {
    const body = req.body;
...
authRouter.get('/auth/pairing/session/:id', requireApiKey, asyncHandler(async (req, res) => {
...
authRouter.post('/auth/pairing/revoke', requireApiKey, asyncHandler(async (req, res) => {
    const body = req.body;
    const sessionId = typeof body?.pairing_session_id === 'string' ? body.pairing_session_id.trim() : '';
    if (!sessionId)
        throw new HttpError('pairing_session_id is required', 400);
...
authRouter.get('/auth/trusted-devices', requireApiKey, asyncHandler(async (req, res) => {
    const devices = await listTrustedDevices(req.cloudAuth?.farmId ?? null);
    jsonOk(res, { devices });
}));
authRouter.post('/auth/trusted-devices/revoke', requireApiKey, asyncHandler(async (req, res) => {
    const body = req.body;
