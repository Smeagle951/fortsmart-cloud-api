/**
 * DDL do módulo plantio: cada comando em query separada para que um erro
 * (ex.: phenology/geo já existentes com outro formato) não impeça criar
 * planting_cv_records, etc.
 */
const PLANTING_DDL_STEPS = [
...
export async function ensurePlantingModuleTables(client) {
    let step = 0;
    for (const sql of PLANTING_DDL_STEPS) {
        step += 1;
        try {
            ...
            console.warn(`[fortsmart-cloud-api] ensurePlantingModuleTables step ${step}/${PLANTING_DDL_STEPS.length}: ${msg}`);
        }
    }
}
//# sourceMappingURL=ensurePlantingSchema.js.map