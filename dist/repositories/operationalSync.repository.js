...
function baseLinked(record, farmId) {
    return {
...
export function getOperationalSpec(module) {
    return specs[module];
}
export async function logSync(client, farmId, opts) {
...
export async function upsertOperationalRecords(client, module, farmId, records, deviceId) {
...
async function upsertMonitoringReport(client, farmId, record, deviceId) {
    ...
    for (let i = 0; i < points.length; i += 1) {
        ...
        try {
            const pointId = await upsertGeneric(client, 'monitoring_points', {
            ...
            for (let j = 0; j < occurrences.length; j += 1) {
                ...
                if (typeof rawOccurrence !== 'object' || rawOccurrence === null || Array.isArray(rawOccurrence))
                    continue;
                const occurrence = rawOccurrence;
                const occurrenceLocalId = localId(occurrence) || `${pointLocalId}:occurrence:${j}`;
                try {
                    const occurrenceId = await upsertGeneric(client, 'monitoring_occurrences', {
...
async function upsertMonitoringRecommendation(client, farmId, occurrenceId, occurrenceLocalId, raw, mapping, deviceId) {
    ...
    try {
        const id = await upsertGeneric(client, 'monitoring_recommendations', {
            local_id: local,
            farm_id: farmId,
...
async function upsertMonitoringImages(client, farmId, reportId, pointId, occurrenceId, raw, fallbackPrefix, mapping, deviceId) {
    ...
    for (let i = 0; i < raw.length; i += 1) {
        ...
        try {
            const id = await upsertGeneric(client, 'monitoring_images', {
                local_id: lid,
                farm_id: farmId,
                monitoring_report_id: reportId,
                monitoring_point_id: pointId,
...
export async function upsertGeneric(client, table, values, jsonColumns) {
    const keys = Object.keys(values);
    const columns = keys.join(', ');
    const placeholders = keys.map((_, i) => `$${i + 1}`).join(', ');
    const updateColumns = keys
    ...
    const { rows } = await client.query(`INSERT INTO ${table} (${columns})
     VALUES (${placeholders})
     ON CONFLICT (farm_id, local_id) DO UPDATE SET
       ${updateColumns},
       updated_at = NOW()
    ...
    const id = rows[0]?.id;
    if (!id)
        throw new Error(`${table} upsert returned no id`);
    return id;
}
export async function loadOperationalRows(client, module, farmId) {
    const spec = getOperationalSpec(module);
...
//# sourceMappingURL=operationalSync.repository.js.map