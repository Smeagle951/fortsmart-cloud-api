...
async function logSecurityEvent(client, input) {
    await client.query(`INSERT INTO security_events (farm_id, device_id, event_type, status, ip, user_agent, details)
...
export async function createPairingSession(input) {
    ...
    if (!desktopInstallationId)
        throw new HttpError('desktop_installation_id is required', 400);
    const pool = getPool();
    ...
    try {
        ...
        if (!farmLookup)
            throw new HttpError('farm_cloud_id is required', 400);
        const farmResult = await client.query(`SELECT id, name FROM farms
       WHERE (
          id::text = $1
          OR farm_cloud_id = $1
          OR local_id = $1
       ...
       LIMIT 1`, [farmLookup]);
        ...
        await client.query(`UPDATE pairing_sessions
       SET status = 'revoked', revoked_at = NOW()
       WHERE farm_id = $1 AND desktop_installation_id = $2 AND status = 'pending'`, [farm.id, desktopInstallationId]);
        const pairingCode = randomCode();
        const pairingToken = randomUrlToken();
        const expiresAt = new Date(Date.now() + PAIRING_TTL_MS).toISOString();
        const inserted = await client.query(`INSERT INTO pairing_sessions (
...
export async function consumePairingSession(input) {
    ...
    if (!token && !code)
        throw new HttpError('pairing_token or pairing_code is required', 400);
    if (!deviceId)
        throw new HttpError('device_id is required', 400);
    ...
    try {
       ...
       WHERE status = 'pending' AND expires_at <= NOW()`);
        const found = await client.query(`SELECT ps.id, ps.farm_id, ps.farm_cloud_id, ps.status, ps.expires_at, ps.consumed_at, f.name AS farm_name
       ...
       FOR UPDATE`, [tokenHash, codeHash]);
        ...
        const insertedKey = await client.query(`INSERT INTO api_keys (farm_id, key_hash, key_prefix, name, is_active)
       VALUES ($1, $2, $3, $4, true)
       RETURNING id`, [session.farm_id, apiKeyHash, displayKeyPrefix(rawApiKey), `Mobile ${deviceId.slice(0, 16)}`]);
        await client.query(`INSERT INTO trusted_devices (
       ...
       WHERE id = $1`, [session.id, deviceId, input.userAgent ?? null]);
        if (farmLocalId) {
...
export async function getPairingSessionStatus(input) {
    ...
    const { rows } = await pool.query(`SELECT id,
     ...
     WHERE id = $1 AND farm_id = $2`, [input.sessionId, input.apiKeyFarmId]);
    const row = rows[0];
    if (!row)
        throw new HttpError('Pairing session not found', 404);
    return {
        id: row.id,
        status: row.status,
...
export async function revokePairingSession(input) {
    ...
    try {
        await client.query('BEGIN');
        const result = await client.query(`UPDATE pairing_sessions
       SET status = 'revoked', revoked_at = NOW()
       WHERE id = $1 AND farm_id = $2 AND status = 'pending'`, [input.sessionId, input.apiKeyFarmId]);
        if (result.rowCount === 0)
            throw new HttpError('Pairing session not found or not pending', 404);
        await logSecurityEvent(client, {
...
export async function listTrustedDevices(farmId) {
    if (!farmId)
        throw new HttpError('Unauthorized', 401);
    const pool = getPool();
    const { rows } = await pool.query(`SELECT id, device_id, farm_cloud_id, platform, app_version, device_name,
...
export async function revokeTrustedDevice(input) {
    ...
    try {
        await client.query('BEGIN');
        await client.query(`UPDATE trusted_devices
       SET revoked_at = NOW()
       WHERE farm_cloud_id = $1 AND device_id = $2 AND revoked_at IS NULL`, [input.apiKeyFarmId, input.deviceId]);
        await logSecurityEvent(client, {
            farmId: input.apiKeyFarmId,
...
export async function validateDeviceBinding(input) {
    const pool = getPool();
    const { rows } = await pool.query(`SELECT device_id, revoked_at
     FROM trusted_devices
     WHERE api_key_hash = $1
     LIMIT 1`, [input.apiKeyHash]);
    const trusted = rows[0];
    if (!trusted)
        return;
    if (trusted.revoked_at)
        throw new HttpError('Dispositivo revogado para esta chave.', 403);
    if (!input.deviceId?.trim())
        throw new HttpError('device_id is required for paired API key', 401);
    ...
    await pool.query(`UPDATE trusted_devices SET last_seen_at = NOW()
     WHERE api_key_hash = $1 AND device_id = $2 AND ($3::uuid IS NULL OR farm_cloud_id = $3::uuid)`, [input.apiKeyHash, trusted.device_id, input.farmId]);
}
