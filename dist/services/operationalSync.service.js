...
import { logSync, upsertOperationalRecords, } from '../repositories/operationalSync.repository.js';
...
async function ensureOperationalCompatibilityColumns(client) {
...
export async function pushOperationalSync(pool, apiKeyId, module, body) {
    ...
    if (module === 'monitoring-report') {
        ...
        console.info('[sync/monitoring-report/push] incoming', JSON.stringify({
    ...
    try {
        ...
        const result = module === 'planting'
            ? await upsertPlantingBundle(client, farmId, body.records, body, body.device_id)
            : await upsertOperationalRecords(client, module, farmId, body.records, body.device_id);
        await logSync(client, farmId, {
            module,
            action: 'push',
            status: result.failed.length === 0 ? 'ok' : 'partial',
            error_message: result.failed.length === 0 ? undefined : `${result.failed.length} item(s) falharam`,
            device_id: body.device_id,
        });
        await client.query('COMMIT');
        if (module === 'planting') {
            const root = result.mapping;
            const count = (k) => root[k] && typeof root[k] === 'object'
                ? Object.keys(root[k]).length
                : 0;
            console.info('[sync/planting/push] persisted', JSON.stringify({
        ...
        if (module === 'monitoring-report') {
            ...
            const count = (k) => root[k] && typeof root[k] === 'object'
                ? Object.keys(root[k]).length
                : 0;
            console.info('[sync/monitoring-report/push] persisted', JSON.stringify({
...
//# sourceMappingURL=operationalSync.service.js.map