...
export async function uploadPlantingImage(pool, input) {
    ...
    const localName = (input.originalFileName && sanitizeSegment(String(input.originalFileName))) ||
        `${sanitizeSegment(input.imageLocalId)}.${ext}`;
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const { rows: plantingRows } = await client.query(`SELECT id::text AS id FROM planting_records
       WHERE farm_id = $1::uuid AND local_id = $2 AND deleted_at IS NULL
       LIMIT 1`, [input.farmId, input.plantingLocalId]);
        const plantingRecordId = plantingRows[0]?.id ?? null;
        if (!plantingRecordId) {
            throw new HttpError('Registro de plantio não encontrado na nuvem. Envie primeiro POST /sync/planting/push para este plantio.', 400);
        }
        ...
        const { rows: upserted } = await client.query(`INSERT INTO planting_images (
...
//# sourceMappingURL=plantingMedia.service.js.map