import { listDecisionInsightsByFarm, } from '../repositories/decisionInsightsSync.repository.js';
function num(value) {
    if (value == null)
        return 0;
...
function normalizeRiskLevel(level) {
    ...
    if (s.includes('crit'))
        return 'critico';
    if (s.includes('alt') || s.includes('high'))
        return 'alto';
    if (s.includes('med'))
        return 'medio';
    if (s.includes('baix') || s.includes('low'))
        return 'baixo';
...
function organismLabel(row) {
    const snap = row.decision_engine_snapshot_json;
    const source = snap?.source;
    return (source?.subtipo?.toString() ??
        source?.organism_name?.toString() ??
        row.organism_id);
}
function roiFromRow(row) {
    const econ = row.economic_analysis_json;
    if (!econ || typeof econ !== 'object')
        return null;
    const presc = econ.prescription;
...
export function mapRowToDesktopDto(row) {
    const evidence = Array.isArray(row.evidence_json) ? row.evidence_json : [];
    const snapshot = row.decision_engine_snapshot_json &&
        ...
        : {};
    const economic = row.economic_analysis_json &&
...
function plotKey(row) {
    return row.talhao_cloud_id ?? row.talhao_local_id ?? 'unknown';
}
export async function loadWindowsDecisionInsights(pool, farmCloudId) {
    ...
    for (const row of rows) {
        ...
        if (isHighRisk(level))
            highRiskCount++;
        if (isCriticalRisk(level))
            criticalRiskCount++;
        if (lossHa > maxLossBrlHa)
            maxLossBrlHa = lossHa;
        if (lossTotal > maxLossTotalBrl)
            maxLossTotalBrl = lossTotal;
        const key = plotKey(row);
        const agg = byPlot.get(key) ??
            {
        ...
        if (lossHa > agg.maxLossBrlHa)
            agg.maxLossBrlHa = lossHa;
...
//# sourceMappingURL=windowsDecisionInsights.service.js.map