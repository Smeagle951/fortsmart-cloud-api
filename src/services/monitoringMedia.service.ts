...
export async function uploadMonitoringImage(
  pool: Pool,
  input: MonitoringImageUploadInput,
): Promise<MonitoringImageUploadResult> {
  ...
  const localName =
    (input.originalFileName && sanitizeSegment(input.originalFileName)) ||
  ...
  try {
    ...
    if (input.occurrenceLocalId) {
      const { rows } = await client.query<{ id: string; monitoring_report_id: string | null; monitoring_point_id: string | null }>(
        `SELECT id, monitoring_report_id, monitoring_point_id
         FROM monitoring_occurrences
         WHERE farm_id = $1 AND local_id = $2 AND deleted_at IS NULL
         LIMIT 1`,
        [input.farmId, input.occurrenceLocalId],
      );
    ...
    if (!pointId && input.pointLocalId) {
      const { rows } = await client.query<{ id: string; monitoring_report_id: string | null }>(
        `SELECT id, monitoring_report_id
         FROM monitoring_points
         WHERE farm_id = $1 AND local_id = $2 AND deleted_at IS NULL
         LIMIT 1`,
        [input.farmId, input.pointLocalId],
      );
    ...
    if (!reportId && input.reportLocalId) {
      const { rows } = await client.query<{ id: string }>(
        `SELECT id FROM monitoring_reports WHERE farm_id = $1 AND local_id = $2 AND deleted_at IS NULL LIMIT 1`,
        [input.farmId, input.reportLocalId],
      );
...
export async function resolveFarmIdForApiKey(
  pool: Pool,
  apiKeyId: string,
  farmCloudId: string,
): Promise<string> {
  const { rows } = await pool.query<{ farm_id: string | null }>(
