...
function numberField(root: Record<string, unknown>, key: string): number {
  const n = Number(root[key] ?? 0);
  return Number.isFinite(n) ? Math.trunc(n) : 0;
}

export async function pushMonitoringSync(
  pool: Pool,
  apiKeyId: string,
  body: MonitoringPushBody,
): Promise<{ payload_id: string; synced_at: string }> {
  ...
  try {
    ...
    const { rows } = await client.query<{ id: string }>(
      `INSERT INTO monitoring_payloads (
    ...
    await client.query(
      `INSERT INTO sync_logs (farm_id, module, entity, cloud_id, action, status, device_id)
       VALUES ($1, 'monitoring', 'payload', $2, 'push', 'ok', $3)`,
      [farmId, rows[0]?.id ?? null, typeof body.device_id === 'string' ? body.device_id : null],
    );
