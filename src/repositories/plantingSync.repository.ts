...
async function resolvePlotCloudId(
  ...
  if (pc) return pc;
  if (!plotLocalId) return null;
  const { rows } = await client.query<{ id: string }>(
    `SELECT id::text AS id FROM plots
...
async function resolvePlantingRecordId(
  client: PoolClient,
  farmId: string,
  plantingLocalId: string | null,
): Promise<string | null> {
  if (!plantingLocalId) return null;
  const { rows } = await client.query<{ id: string }>(
    `SELECT id::text AS id FROM planting_records
     WHERE farm_id = $1::uuid AND local_id = $2 AND deleted_at IS NULL
     LIMIT 1`,
    [farmId, plantingLocalId],
  );
  return rows[0]?.id ?? null;
}
...
export async function upsertPlantingBundle(
  client: PoolClient,
  farmId: string,
  records: Record<string, unknown>[],
  body: OperationalPushBody,
  deviceId: string,
): Promise<PlantingBundleResult> {
  ...
  const mPlant = mapping.planting_records as Record<string, string>;
  const mStand = mapping.stand_evaluations as Record<string, string>;
  const mCv = mapping.cv_records as Record<string, string>;
  const mCalib = mapping.calibration_records as Record<string, string>;
  const mPhen = mapping.phenology_records as Record<string, string>;
  const mGeo = mapping.geo_exports as Record<string, string>;
  const mImg = mapping.images as Record<string, string>;

  const seen = {
    stand: new Set<string>(),
    cv: new Set<string>(),
    calib: new Set<string>(),
    phen: new Set<string>(),
    geo: new Set<string>(),
    img: new Set<string>(),
  };

  async function upsertOneChild(
    table: string,
    values: Record<string, unknown>,
    jsonCols: string[],
    mapBucket: Record<string, string>,
    seenSet: Set<string>,
    entity: string,
  ): Promise<void> {
    const lid = str(values, 'local_id');
    if (!lid) return;
    if (seenSet.has(lid)) return;
    seenSet.add(lid);
    try {
  ...
  for (const record of records) {
    ...
    try {
      ...
      const resolvedPlot = await resolvePlotCloudId(
        client,
        farmId,
        plotLocal,
        str(record, 'plot_cloud_id'),
      );
      const enriched = { ...record, plot_cloud_id: resolvedPlot ?? record.plot_cloud_id };
      const values = spec.buildValues(enriched as Record<string, unknown>, farmId);
      ...
      if (!plantingId) throw new Error('planting upsert returned no id');
      mPlant[plocal] = plantingId;

      const plotL = str(record, 'plot_local_id');

      const processStand = async (child: Record<string, unknown>) => {
        const lid = localId(child) || str(child, 'id') || '';
        if (!lid) return;
        await upsertOneChild(
          'plant_stand_records',
          {
            local_id: lid,
            farm_id: farmId,
      ...
      const processCv = async (child: Record<string, unknown>) => {
        const lid = localId(child) || str(child, 'id') || '';
        if (!lid) return;
        await upsertOneChild(
          'planting_cv_records',
          ...
          ['raw_payload', 'detailed_metrics', 'calculation_details'],
          mCv,
          seen.cv,
          'planting_cv_record',
        );
      ...
      const processCalib = async (child: Record<string, unknown>) => {
        const lid = localId(child) || str(child, 'id') || '';
        if (!lid) return;
        await upsertOneChild(
          ...
          mCalib,
          seen.calib,
          'planting_calibration_record',
        );
      };

      const processPhen = async (child: Record<string, unknown>) => {
        const lid = localId(child) || str(child, 'id') || '';
        if (!lid) return;
        await upsertOneChild(
          'phenology_records',
      ...
      const processGeo = async (child: Record<string, unknown>) => {
        const lid = localId(child) || str(child, 'id') || '';
        if (!lid) return;
        await upsertOneChild(
          'geo_exports',
      ...
      const processImg = async (child: Record<string, unknown>) => {
        const lid = localId(child) || str(child, 'id') || '';
        if (!lid) return;
        await upsertOneChild(
          'planting_images',
      ...
      for (const s of standArr) {
        if (typeof s === 'object' && s !== null && !Array.isArray(s)) await processStand(s as Record<string, unknown>);
      }
      const cvArr = Array.isArray(record.cv_records) ? record.cv_records : [];
      for (const s of cvArr) {
        if (typeof s === 'object' && s !== null && !Array.isArray(s)) await processCv(s as Record<string, unknown>);
      }
      const calArr = Array.isArray(record.calibration_records) ? record.calibration_records : [];
      for (const s of calArr) {
        if (typeof s === 'object' && s !== null && !Array.isArray(s)) await processCalib(s as Record<string, unknown>);
      }
      const phenArr = Array.isArray(record.phenology_records) ? record.phenology_records : [];
      for (const s of phenArr) {
        if (typeof s === 'object' && s !== null && !Array.isArray(s)) await processPhen(s as Record<string, unknown>);
      }
      const geoArr = Array.isArray(record.geo_exports) ? record.geo_exports : [];
      for (const s of geoArr) {
        if (typeof s === 'object' && s !== null && !Array.isArray(s)) await processGeo(s as Record<string, unknown>);
      }
      const imgArr = Array.isArray(record.images) ? record.images : [];
      for (const s of imgArr) {
        if (typeof s === 'object' && s !== null && !Array.isArray(s)) await processImg(s as Record<string, unknown>);
      }
  ...
  const mergeOrphans = async (
    list: Record<string, unknown>[] | undefined,
    processor: (row: Record<string, unknown>, parentId: string | null) => Promise<void>,
  ) => {
    if (!list) return;
...
export async function loadPlantingWindowsPayload(
  client: PoolClient,
  farmId: string,
): Promise<Record<string, unknown>> {
  ...
  const { rows: plantings } = await client.query<Record<string, unknown>>(
    `SELECT pr.*,
     ...
     ORDER BY pr.planting_date DESC NULLS LAST, pr.updated_at DESC`,
    [farmId],
  );
  ...
  const plotMap = new Map<string, PlotBucket>();

  const plotKeyOf = (pr: Record<string, unknown>) =>
    String(pr.resolved_plot_id ?? pr.plot_cloud_id ?? pr.plot_local_id ?? 'unknown');
  ...
  for (const pr of plantings) {
    ...
    if (pd) latestDates.push(pd);

    const pk = plotKeyOf(pr);
    if (!plotMap.has(pk)) {
      plotMap.set(pk, {
        plot_id: (pr.resolved_plot_id as string) ?? (pr.plot_cloud_id as string) ?? null,
        plot_local_id: (pr.plot_local_id as string) ?? (pr.resolved_plot_local_id as string) ?? null,
    ...
    if (!plot.subareas.has(sk)) {
      plot.subareas.set(sk, {
        subarea_id: (pr.subarea_cloud_id as string) ?? null,
        subarea_local_id: (pr.subarea_local_id as string) ?? null,
