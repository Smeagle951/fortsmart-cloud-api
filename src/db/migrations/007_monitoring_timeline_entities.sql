...
BEGIN
  IF EXISTS (
    ...
    WHERE table_name = 'monitoring_recommendations'
      AND column_name = 'monitoring_occurrence_id'
  ) THEN
    EXECUTE 'UPDATE monitoring_recommendations
             SET occurrence_id = monitoring_occurrence_id
             WHERE occurrence_id IS NULL';
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_monitoring_recommendations_farm_id
  ON monitoring_recommendations (farm_id);
...
CREATE INDEX IF NOT EXISTS idx_monitoring_images_farm_id
  ON monitoring_images (farm_id);
CREATE INDEX IF NOT EXISTS idx_monitoring_images_report_id
  ON monitoring_images (monitoring_report_id);
